...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformedProposals = proposals?.map(proposal => {
      ...
      const chat = proposal.chat?.[0] // Acceder al primer elemento del array chat
      const intercambio = chat?.intercambio?.[0] // Acceder al primer elemento del array intercambio
