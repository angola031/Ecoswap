    ...
    try {
        ...
        if (action === 'completado') {
            ...
            if (it.producto_solicitado_id) {
                await admin.from('producto').update({ estado_publicacion: 'intercambiado' }).eq('producto_id', it.producto_solicitado_id)
            }
            if (it.producto_ofrecido_id) {
                await admin.from('producto').update({ estado_publicacion: 'intercambiado' }).eq('producto_id', it.producto_ofrecido_id)
            }
            ...
            for (const uid of participantes) {
                ...
                await admin.from('usuario').update({ total_intercambios: total }).eq('user_id', uid)
