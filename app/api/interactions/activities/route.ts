...
import { getUserActivities } from '@/lib/interactions-queries'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await getUserActivities(userId, limit)

    if (!result.success) {
      return NextResponse.json({ error: result.error }, { status: 500 })
    }

    return createSuccessResponse({ activities: result.data })
