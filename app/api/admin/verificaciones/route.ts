...
export async function GET(req: NextRequest) {
    ...
    const { data: valids } = await supabaseAdmin
    ...
    for (const v of valids || []) {
        ...
        const files: Record<string, string | null> = {
            cedula_frente: supabaseAdmin.storage.from('Ecoswap').getPublicUrl(prefix + 'cedula_frente.jpg').data.publicUrl,
            cedula_reverso: supabaseAdmin.storage.from('Ecoswap').getPublicUrl(prefix + 'cedula_reverso.jpg').data.publicUrl,
            selfie_validacion: supabaseAdmin.storage.from('Ecoswap').getPublicUrl(prefix + 'selfie_validacion.jpg').data.publicUrl,
        }
...
export async function POST(req: NextRequest) {
    ...
    if (action === 'aprobar') {
        const { error: e1 } = await supabaseAdmin.from('usuario').update({ verificado: true }).eq('user_id', userId)
        ...
        const { data: u } = await supabaseAdmin.from('usuario').select('user_id').eq('user_id', userId).single()
        if (u) {
        ...
        if (action === 'rechazar') {
            const { error: e1 } = await supabaseAdmin.from('usuario').update({ verificado: false }).eq('user_id', userId)
            if (e1) return NextResponse.json({ error: e1.message }, { status: 400 })
            const { error: e2 } = await supabaseAdmin
                .from('validacion_usuario')
