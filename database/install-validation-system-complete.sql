-- Script completo de instalación del sistema de validación y calificación
-- Este script evita errores de ambigüedad y instala todo el sistema de una vez
...
-- 10. Función para obtener estadísticas de usuario (CORREGIDA - sin ambigüedad)
...
-- 11. Trigger para actualizar estadísticas cuando se completa un intercambio
...
-- 12. Crear trigger para actualizar estadísticas
...
-- 13. Trigger para actualizar calificación promedio cuando se agrega una calificación
...
-- 14. Crear trigger para la tabla calificacion
...
BEGIN
    ...
    ELSE
        RAISE NOTICE '✅ Función get_intercambio_validations existe';
    END IF;
    
    -- Verificar que la función get_user_stats se creó correctamente
    IF NOT EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'get_user_stats') THEN
        RAISE EXCEPTION '❌ La función get_user_stats no se creó correctamente';
    ELSE
        RAISE NOTICE '✅ Función get_user_stats existe';
    END IF;
